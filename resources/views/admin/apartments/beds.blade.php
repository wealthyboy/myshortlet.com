...
<div class="bedroom-{{ $key + 1 }} mt-3">
    ...
    <label for="bedroom-{{ $bedroom->id }}-{{ $apartment->id }}" class="radio-inline">
        ...
        <div class="bed-count">
            <input name="bed_count[{{ $apartment->id }}][{{ $bedroom->id }}]" placeholder="Number of beds" class="form-control bed-qty" value="222{{ $helper->check(optional($apartment)->bedrooms, $bedroom->id,'bed_count')   }}" type="number">
        </div>
    </label>
    @endforeach
</div>
...
<div class="bedroom-{{ $key + 1 }} d-none  {{ $key }}">
    ...
    <label for="bedroom-{{ $bedroom->id }}" class="radio-inline">
        ...
        <div class="bed-count">
            <input name="bed_count[{{ $apartment->id }}][{{ $bedroom->id }}]" placeholder="Number of beds" class="form-control  bed-qty" value="3333{{ $helper->check(optional($apartment)->bedrooms, $bedroom->id,'bed_count')   }}" type="number">
        </div>
    </label>
    @endforeach
</div>
